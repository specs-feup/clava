...
/**
 * Measures CompleteInput import, real Clava TU/App construction, traversal and code generation.
 *
 * <p>The input syntax is intentionally usable by the native driver:</p>
 *
 * <pre>
 * CompleteBenchmark fixture source.c text,eager,lazy text.dump eager.fb lazy.fb
 * CompleteBenchmark --fixture=fixture --source=source.c --modes=text,eager,lazy \
...
final class CompleteBenchmark {
    ...
    enum Mode {
        ...
        static Mode parse(String value) {
            ...
            default -> throw new IllegalArgumentException("Unknown CompleteInput mode: " + value);
            };
        }
    ...
    static LoadedApp loadAppDetailed(Path source, Mode mode, Path dump, boolean textZstd) throws Exception {
        CompleteInput.Result result = read(mode, dump, textZstd);
    ...
    static CompleteInput.Result read(Mode mode, Path dump, boolean textZstd) throws Exception {
        return switch (mode) {
        case TEXT -> CompleteInput.readText(dump, textZstd);
        case EAGER -> CompleteInput.read(dump, false);
        case LAZY -> CompleteInput.read(dump, true);
        };
    }
    ...
    private static Counters counters(CompleteInput.Result result) {
    ...
    private static Input parseArgs(String[] args) {
        if (args.length == 0 || contains(args, "--help")) {
            throw new IllegalArgumentException("usage: CompleteBenchmark [--fixture=name] [--source=file] "
