...
export default class Inliner {
  ...
  private processBodyToInline(
    ...
    this.renameLabels($newNodes, $call.getAncestor("function") as FunctionJp);
  }

  /**
   * Labels need to be renamed, to avoid duplicated labels.
   */
  private renameLabels($newNodes: Scope, $destFunction: FunctionJp): void {
    // Maps label names to new LabelDecl
    const newLabels: Record<string, LabelDecl> = {};

    // The destination function may already contain labels generated by earlier
    // inlines (e.g., from a previous Inliner instance, whose counter starts at
    // zero), so generated names are checked against it instead of relying on
    // the instance counter for uniqueness.
    // LabelDecl join points are not traversable through Query.searchFrom, so
    // existing labels are collected from the goto/label statements that
    // reference them.
    const destLabelNames = new Set<string>();
    for (const jp of Query.searchFrom($destFunction, Joinpoint, {
      self: ($jp: LaraJoinPoint) =>
        $jp instanceof GotoStmt || $jp instanceof LabelStmt,
    })) {
      const $jp = jp as GotoStmt | LabelStmt;
      const $destLabelDecl = $jp instanceof GotoStmt ? $jp.label : $jp.decl;
      destLabelNames.add($destLabelDecl.name);
    }
    ...
    for (const jp of Query.searchFrom($newNodes, Joinpoint, {
      ...
      if ($newLabelDecl === undefined) {
        let newLabelName = this.createNewLabelName($labelDecl.name);
        while (destLabelNames.has(newLabelName)) {
          newLabelName = this.createNewLabelName(newLabelName);
        }
        destLabelNames.add(newLabelName);
