...
static llvm::cl::opt<int>
    UserSystemHeaderThresholdOption("system-header-threshold",
...
int main(int argc, const char *argv[]) {
    ...
    if (auto E = OptionsParser.takeError()) {
        ...
        llvm::errs() << "Problem while creating options parser: "
                     << toString(std::move(E)) << "\n";
        return -1;
    }
    ...
    // Making it static/global because I do not know how to create actions with
    // arbitrary arguments using newFrontendActionFactory
    DumpResources::init(UserIdOption.getValue(),
                        UserSystemHeaderThresholdOption.getValue());

    int includesReturnCode =
        Tool.run(newFrontendActionFactory<DumpIncludesAction>().get());
    ...
    if (includesReturnCode != 0) {
        std::cout << "Error while running IncludeFinderAction (return code "
                  << includesReturnCode << ")";
        returnCode = 1;
    }

    if (dumpReturnCode != 0) {
        std::cout << "Error while running DumpAstAction (return code "
                  << dumpReturnCode << ")";
        returnCode = 1;
    }
