import MargotStringsGen from "@specs-feup/antarex/api/margot/codegen/_internal/MargotStringsGen.js";
import Clava from "@specs-feup/clava/api/clava/Clava.js";
import MargotConfig from "@specs-feup/antarex/api/margot/config/MargotConfig.js";
...
MargotCodeGen.fromConfig = function (config, blockName) {
    ...
    checkInstance(
        config,
        MargotConfig,
        "MargotCodeGen.fromConfig(config)",
        "MargotConfig"
    );
...
MargotCodeGen.prototype._includeHeader = function ($joinpoint) {
    ...
    $parentFile.addInclude(includeCode, true);
};
...
MargotCodeGen.prototype.update = function ($joinpoint) {
    ...
    var code = this._stringsGen.update(
        this._block,
        this._vars,
        this._features,
        additionalCode
...
MargotCodeGen.prototype.startMonitors = function ($joinpoint) {
    ...
    var monitorCode = this._stringsGen.startAllMonitors(
        this._block,
        arrayFromArgs(arguments, 1)
    );
...
MargotCodeGen.prototype.stopMonitors = function ($joinpoint) {
    ...
    var monitorCode = this._stringsGen.stopAllMonitors(
        this._block,
        arrayFromArgs(arguments, 1)
    );
...
MargotCodeGen.prototype.startMonitor = function ($joinpoint, monitorName) {
    ...
    var monitorCode = this._stringsGen.startMonitor(
        this._block,
        monitorName,
        arrayFromArgs(arguments, 2)
    );
    $joinpoint.insert("before", monitorCode);
...
MargotCodeGen.prototype.stopMonitor = function ($joinpoint, monitorName) {
    ...
    var monitorCode = this._stringsGen.stopMonitor(
        this._block,
        monitorName,
        arrayFromArgs(arguments, 2)
    );
 ...
 * */
MargotCodeGen.prototype.monitorLog = function (
    $joinpoint,
    startArgs,
    stopArgs
) {
...
MargotCodeGen.prototype.init = function ($joinpoint) {
    ...
    if (scope === undefined) {
        throw (
            "Could not find a suitable scope to insert the initialization code from the join point provided: \n" +
            $joinpoint.code
        );
    }
