...
*/
export function getExternalDeclsOf($statement, $FunctionDecls) {
    var $varDecls = [];
    var $externDecls = [];
...
*/
export function getReferencedParameters($statement) {
    var $varDecls = [];
    var $parameters = [];
    var $FunctionDecls = [];
    var $stmt = getStmtToVisitForExternalDecls($statement);
    getRefVarsInStmt($stmt, $varDecls, $FunctionDecls);
    for (var $f of $varDecls) {
        if (isParameterDeclaration($f)) insertInUsingLocation($parameters, $f);
    }
    return $parameters;
}
...
*/
export function isOutsideDeclaration($aDecl, $statement) {
...
*/
export function getStmtToVisitForExternalDecls($statement) {
    ...
    switch ($astname) {
        ...
        case "CXXOperatorCallExpr":
            NYI(
                "getStmtToVisitForExternalDecls,  CXXOperatorCallExpr detected, code = " +
                    $statement.code
            );
            break;
...
/**     
  ...
  */
export function getRefVarsInStmt($statement, $refVars, $FunctionRefs) {
    var $astname = $statement.astName;
    switch ($astname) {
        case "NullStmt":
        case "BreakStmt":
        ...
        case "CXXOperatorCallExpr":
            NYI(
                "getRefVarsInStmt,  CXXOperatorCallExpr detected, code = " +
                    $statement.code
            );
            break;
...
/**     
  ...
  */
export function getRefVarsInExpr($expr, $refVars, $FunctionRefs) {
    ...
    switch ($jpt) {
        ...
        case "call":
            ...
            if (isaFunction(funcDecl))
                insertInUsingLocation($FunctionRefs, funcDecl);
...
*/
export function insertInUsingLocation(tabDecl, $stmt) {
    for (var f of tabDecl) if (f.location === $stmt.location) return;
