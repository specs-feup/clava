...
function LibVC($func, compilerSetup, name) {
    ...
    checkTrue(
        $func.hasDefinition,
        "$func should be a function definition",
        "LibVC"
    );
    checkDefined(
        $func.getAncestor("file"),
        "$func.getAncestor('file')",
        "LibVC"
    );
    ...
    Clava.getProgram().addExtraIncludeFromGit(
        "https://github.com/skeru/libVersioningCompiler.git",
        "include/"
    );
    ...
    this._funcPath = Io.getAbsolutePath(
        Clava.getWeavingFolder(),
        $func.getAncestor("file").relativeFilepath
    );
    this._type = "";
    this._typedef = "";
    this._makeType($func);
...
LibVC.prototype._makeDefaultCompilerInit = function () {
    var code =
        "vc::compiler_ptr_t " +
        this._compName +
        " = vc::make_compiler<vc::SystemCompiler>(";
...
LibVC.prototype._makeValueCode = function (opt, $joinpoint) {
    if (opt.l) {
        ...
        const gvc = _LVC_GET_VALUE_CODE_(opt, $joinpoint);
        return gvc.code;
    }
};

function OptionListTemplate(name, optionsCode, optionsList) {
    return `
${optionsCode}
vc::opt_list_t ${name} = {
    ${optionsList}
...
function BuilderCheckTemplate(versionName, errorCode) {
    return `
if (!${versionName}) {
...
function CompileCheckTemplate(versionName, errorCode) {
    return `
if (!${versionName}_ok) {
...
function CompileTemplate(versionName, checkCode) {
    return `
...
function LoadTemplate(name, typedef, type, versionName) {
    return `
${typedef}
...
LibVC.prototype.replaceCall = function ($joinpoint) {
    ...
    if (Weaver.isJoinPoint($joinpoint, "call")) {
        $joinpoint.setName(this._symbolName);
    } else if (Weaver.isJoinPoint($joinpoint, "tag")) {
        var $stmt = $joinpoint.target;
        _LVC_CHANGE_IN_TAG_($stmt, this._symbolName);
    } else {
        throw (
            "Expected variable to be a join point of type 'call' or 'tag', but it's of type '" +
            $joinpoint.joinPointType +
            "'"
        );
    }
};
