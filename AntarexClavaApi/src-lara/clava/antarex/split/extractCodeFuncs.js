...
export var $EXTRACT_CODE_PRAGMA = "EXTRACT";
var $ID_EXTRACTED_CODE = "INRIA_EXTRACTED_CODE_";
var $CPT_EXTRACTED_CODE = -1;
var $CURRENT_ID_EXTRACTED_CODE;
export var $ISOLATE_PRAGMA = "ISOLATE";
export var $TMP_PRAGMA = "INRIA_TMP_PRAGMA";

/**
  @return true if a pragma ($apragma) is named $EXTRACT_CODE_PRAGMA
*/
export function isBeginIsolateCodePragma($apragma) {
...
*/
export function mkNewExtractedCodeIdentifier() {
    $CPT_EXTRACTED_CODE++;
...
*/
export function declareNewIsolateCodeFunctionOrMethod($stmt, name) {
    ...
    for (var $p of $prags) {
        if ($p.name !== $TMP_PRAGMA)
            if ($p.name !== $EXTRACT_CODE_PRAGMA)
                if (npragmas === "") npragmas = $p.code;
                else npragmas = npragmas + "\n" + $p.code;
    }
    var vcode = $stmt.code;
    $nfile.insert(
        "before",
...
*/
export function declareIsolateFunctionOrMethod($funcOrMethodDecl, b) {
...
*/
export function genIsolateCodeVersions($target, n) {
    var table = [];
...
*/
export function genSelectorCode($target, tabNewNames) {
    var nb = tabNewNames.length;
    var $body = $target.body;
    if (nb === 1) {
        if (isaFunction($target)) $body.replaceWith(";");
        else $target.detach();
    } else {
        $target.insert(
            "before",
            "int " + getExtractCodeSymbolSelectedId() + "= 1;"
        );
        var codeSelector =
        ...
        for (var i = 1; i <= nb; i++) {
            var code =
                ...
                ":" +
                "return " +
                mkCallToRenamed($target, tabNewNames[i - 1]) +
                ";";
...
*/
export function genDeclCreatedObjects($target, tabNewNames) {
    var nb = tabNewNames.length;
    if (nb === 1) return;
    var $where;
    if (isaFunction($target)) $where = $target;
    else $where = $target.declarationJp;
    var returnType = getStrReturnType($target);
    var codeParams = getStrParams($target);
    for (var i = 1; i <= nb; i++) {
        var codeDecl =
            returnType + " " + tabNewNames[i - 1] + " " + codeParams + ";";
...
*/
export function isBeginIsolatePragma($apragma) {
    return $apragma.name === $ISOLATE_PRAGMA;
}

export function getAssociatedName($target) {
    var $prags = $target.pragmas;
    for (var $p of $prags) if ($p.name === $TMP_PRAGMA) return $p.content;
    return undefined;
}
