...
export var $SPLITPRAGMA = "SPLITLOOP";
...
export function satisfiesSplitLoopsCriteria(aloop) {
...
export function tryToNormalizeLoopForSplitting(aloop) {
    if (aloop.astName === "CXXForRangeStmt") return false;
    if (aloop.kind !== "for") return false;
...
/**
  ...
  **/
export function getSplitLoopMangleNameOf($vFunction, $ident, $refParams) {
    var line = "\n";
    var $vclass = getStrClassOf($vFunction);
    if ($vclass !== "") $vclass = $vclass + "::";

    var srcFile =
        Io.getTempFolder() + getFileSeparator() + "INRIA_IRISA_MANGLING.cpp";
    Io.writeFile(srcFile, "// generated " + line);
    var Includes = getImportedIncludes(
        $vFunction.getAncestor("file"),
        $vFunction.astId
    );
    ...
    var newmangling = getMangledName(
        srcFile,
        $vFunction.root.includeFolders,
        $ident
    );
...
export function genLoaderLib($vFunction, $nbLibs, $refParams) {
    ...
    var manglingSymbol = getSplitLoopMangleNameOf(
        $vFunction,
        $CURRENT_ID_SPLITTEDLOOP,
        rparams,
        useCC
    );
...
`
    ...
    $currentFunction.insert(
        "before",
        "void " + getCurrentLoadLibrary() + "(const char* aLib){"
    );
    $currentFunction.insert(
        "before",
        "void (*" + $CURRENT_ID_SPLITTEDLOOP + ")" + theParameters + ";"
    );
    ...
    $currentFunction.insert(
        "before",
        "int " + getCurrentCpt() + "[" + $nbLibs + "] = {" + TT + "};"
    );
    $currentFunction.insert(
        "before",
        "#define " + $CURRENT_ID_SPLITTEDLOOP + " " + manglingSymbol
    );

    // gen declaration of the includes.
    $currentFunction.addInclude("time.h", true);
    $currentFunction.addInclude("chrono", true);
    $currentFunction.addInclude("dlfcn.h", true);
    $currentFunction.addInclude("iostream", true);
    $currentFunction.addInclude("stdlib.h", true);
    ...
    $vfile.addInclude($INRIA_SPLIT_LOOP_BEST_CODE_FILE, false);
}

export function AddGlobalDecls($theLoop, $nbLibs, $refParams) {
    ...
    $currentFunction.insert(
        "before",
        "int " + getLearning() + "= 1," + getSelectedId() + "= 0;"
    );
}

export function declareLoopAsNewFunctionOrMethod(
    $theLoop,
    ...
    if ($theLoop.stepValue > 0) $theLoopCopy.condRelation = "<";
    else $theLoopCopy.condRelation = ">";
...
*/
export function getNbIterations(lbound, ubound, stepValued, vop) {
...
/**
  ...
  */
export function replaceLoop($loop, $nbLoops, $referencedParams) {
    var vobj = "";
    ...
    for (var i = nbLibs; i > 0; i--) {
        ...
        if (isFirstIndex) i_ubound = coefubound;
        else i_ubound = "(" + coefubound + "*" + vindex + ")";
        if (stepValued < 0) i_ubound = $loop.initValue + "-" + i_ubound;

        if (isFirstIndex) prefix = "auto ";
        else prefix = "";
        ...
        $loop.insert(
            "before",
            loadLib + "(" + '"' + getSplitLoopLib(vindex) + '"' + VCALL_END
        );
        $loop.insert(
            "before",
            VCALL_ITER_BEGIN + lbound + "," + i_ubound + strParams + VCALL_END
        );
        ...
        $loop.insert(
            "before",
            'LB_TRACE(cout << " MODE_LEARNING SPLIT " << ' +
                vindex +
                ' << " took " << ' +
                tloop_took +
                ' << " ms to run." << endl;);'
    ...
    $loop.insert("after", "} ");
    if (vop === Relation.LE) vmax = vmax + "+1";
    else if (vop === Relation.GE) vmax = vmax + "-1";
    $loop.insert(
        "after",
        VCALL_ITER_BEGIN + vmin + "," + vmax + strParams + VCALL_END
    );
    $loop.insert("after", "} ");
    for (var i = nbLibs; i > 0; i--) {
        if (i !== nbLibs) {
        ...
        $loop.insert(
            "after",
            loadLib + "(" + '"' + getSplitLoopLib(i) + '"' + VCALL_END
        );
    ...
    $loop.replaceWith(
        VCALL_ITER_BEGIN + i_ubound + "," + vmax + strParams + VCALL_END
    );
}
...
/**
  ...
  */
export function generateBestChoiceCode() {
...
*/
export function mkNewSplittedLoopName() {
    $CPT_SPLITTEDLOOP++;
...
*/
export function getCurrentSplittedLoopName() {
    return $CURRENT_ID_SPLITTEDLOOP;
}
...
*/
export function getCurrentSplittedFileName() {
...
*/
export function getCurrentLoadLibrary() {
...
*/
export function getSplitLoopLib(vindex) {
...
*/
export function getSplipLoopPragma($s) {
    var p = $s.pragmas;
    for (vp of p) if (vp.name === $SPLITPRAGMA) return vp;
    return undefined;
}
...
*/
export function getFunctionBestId() {
