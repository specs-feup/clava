...
static void print_current_time(void) {
    time_t now = time(NULL);          /* time_t + time() */
    if (now == (time_t)-1) {
        puts("time() failed");
        return;
    }

    struct tm *local = localtime(&now);  /* struct tm + localtime() */
    if (local == NULL) {
        puts("localtime() failed");
        return;
    }

    struct tm copy = *local;  /* Copy struct tm value */
    ...
    time_t round_trip = mktime(&copy);   /* mktime() */
    if (round_trip != (time_t)-1) {
        double diff = difftime(round_trip, now);  /* difftime() */
        printf("Round-trip diff (seconds): %.0f\n", diff);
    }
}

static void measure_cpu_time(void) {
