...
public class TopLevelTypesParser extends GenericLineStreamParser {

    private static final String PARSER_ID = "<Top Level Types>";
    private static final DataKey<Set<String>> TOP_LEVEL_Types = KeyFactory
            .generic("stream_top_level_types", new HashSet<>());

    public static TopLevelTypesParser newInstance() {
        return new TopLevelTypesParser(getDataKey(), newSnippetParser());
    }
    ...
    private TopLevelTypesParser(DataKey<?> key, SnippetParser<?, ?> parser) {
        super(key, parser);
    }
