...
public class DFGUtils {
     ...
     * 
     * @param node
     * @return The parent loop node closest to the given node if it exists; retursn
     ...
     * 
     * @param node
     * @return the loop node that immediately precedes the node if it exists;
     *         returns the node itself otherwise
     */
     ...
     * 
     * @param node
     * @return the Store node that immediately precedes the node if it exists;
     *         returns the node itself otherwise
     */
    ...
    /**
     * Gets a list with all the nodes that form up an index of an array, with as
     * many nodes as the dimension of the array access (e.g., arr[i][j] would return
     * two nodes, i and j) If the index is made of a large expression, it returns
     * the closest node of that expression (e.g., a[i+1*2] would return the node
    ...
    /**
     * Attempts to estimate the number of iterations of a given node. Only
     * guaranteed to work if the function has a predictable number of iterations on
     * each loop in compile-time
     * 
     * @param node
    ...
    /**
     * Gets the total number of array loads in the function's runtime Uses a list of
     * subgraph metrics as a starting point
     * 
     * @param metrics
    ...
    /**
     * Gets the name of the variable used as loop iterator. Only works for loops
    ...
    /**
     * Compares two expressions to see if they refer to the same loop access (e.g.,
     * a[i+1] and a[1+i] have different yet equivalent expressions)
     * 
     * @param n1
     * @param n2
    ...
    /**
     * Gets the expression that forms up a loop index given the first node of the
     * expression (e.g., given node "+" from a[i+1*2], we'd fetch nodes "i", "1",
     * "*" and "2")
     * 
     * @param n
