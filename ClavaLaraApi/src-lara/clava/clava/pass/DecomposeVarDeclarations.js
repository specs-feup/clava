...
export default class DecomposeVarDeclarations extends SimplePass {
    _name = "DecomposeVarDeclarations";
    matchJoinpoint($jp) {
        return ($jp instanceof Vardecl && // Must be a variable declaration
    ...
    transformJoinpoint($vardecl) {
        ...
        const $newInitStmt = ClavaJoinPoints.exprStmt(ClavaJoinPoints.assign(ClavaJoinPoints.varRef($vardecl), $init));
        $vardecl.insertAfter($newInitStmt);
        return new PassResult(this, $vardecl);
    }
}
//# sourceMappingURL=DecomposeVarDeclarations.js.map